...
def plot_soh_comparison(model_name, cycle_labels_test, y_test_soh, y_pred_soh):
    cycle_df = pd.DataFrame(
        {
            "cycle": cycle_labels_test,
            "true_soh": y_test_soh,
            "predicted_soh": y_pred_soh,
        }
    )
...
# Custom classes to handle "time_major" argument issue during loading
try:
...
def test_model(
    ...
    try:
        ...
        print(f"load_model with custom_objects failed: {e}")
        try:
            # Try without custom objects
            model = load_model(model_path, compile=False)
        except Exception as e2:
            print(f"load_model plain failed: {e2}")
            print(f"尝试重新构建 {model_name} 模型并加载权重 (load_weights)...")
            input_shape = (X_test.shape[1], X_test.shape[2])
            try:
                # Rebuild model from code
                model = get_model(model_name, input_shape)
                model.load_weights(model_path)
                print(f"模型 {model_name} 权重加载完成。")
            except Exception as e3:
                print(f"权重加载也失败: {e3}")
                # Special handling for potential LSTM bug in get_model
                if model_name == "LSTM":
                    print("尝试使用 create_lstm_model 修复 LSTM 加载...")
                    try:
    ...
    time_save_path = os.path.join(
        results_dir, f"{exp_name}_{model_name}_prediction_time.txt"
    )
    with open(time_save_path, "w") as f:
        f.write(
            f"Average prediction time per sample: {avg_prediction_time:.6f} seconds\n"
        )
    ...
    metrics_save_path = os.path.join(
        results_dir, f"{exp_name}_{model_name}_evaluation.txt"
    )
...
def plot_metrics_comparison(metrics_dict):
    ...
    for metric in metrics:
        # 获取对应指标的值并排序
        values = np.array(
            [metrics_dict[model][metric.upper()] for model in model_names]
        )
        ...
        plt.figure(figsize=(10, 6))
        bars = plt.barh(
            sorted_model_names, sorted_values, color=cmap(range(len(model_names)))
        )
        plt.xlim(ylim)
