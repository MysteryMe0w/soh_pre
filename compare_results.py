...
def calculate_pso_statistics(pso_results):
    ...
    stats = {
        'mse': {'mean': np.mean(test_mse), 'std': np.std(test_mse), 'min': np.min(test_mse), 'max': np.max(test_mse)},
        'mae': {'mean': np.mean(test_mae), 'std': np.std(test_mae), 'min': np.min(test_mae), 'max': np.max(test_mae)},
        'rmse': {'mean': np.mean(test_rmse), 'std': np.std(test_rmse), 'min': np.min(test_rmse), 'max': np.max(test_rmse)},
        'r2': {'mean': np.mean(test_r2), 'std': np.std(test_r2), 'min': np.min(test_r2), 'max': np.max(test_r2)},
...
def plot_metrics_comparison(baseline, pso_stats, save_dir):
    ...
    for idx, (ax, metric) in enumerate(zip(axes, metrics)):
        ...
        # 为PSO添加误差棒
        ax.errorbar(1, pso_means[idx], yerr=pso_stds[idx], 
...
def plot_prediction_comparison(baseline_dir, pso_results, save_dir):
    ...
    plt.plot(x, baseline_y_test.flatten(), 'o-', label='Ground Truth', 
             color='black', linewidth=2, markersize=4, alpha=0.7)
    plt.plot(x, baseline_y_pred.flatten(), 's--', label='Baseline Prediction', 
             color='#3498db', linewidth=1.5, markersize=4, alpha=0.7)
    plt.plot(x, pso_y_pred.flatten(), '^--', label='PSO Optimized Prediction', 
             color='#e74c3c', linewidth=1.5, markersize=4, alpha=0.7)
    
    plt.xlabel('Sample Index', fontsize=13)
    plt.ylabel('Normalized SOH', fontsize=13)
    plt.title('Prediction Comparison: Baseline vs PSO Optimized', fontsize=15, fontweight='bold')
...
def plot_convergence_all_runs(pso_results, save_dir):
    ...
    for result in pso_results:
        ...
        if result_files:
            with open(result_files[0], 'r') as f:
                ...
                plt.plot(iterations, scores, '-o', label=f'Run {run_id}', 
    ...
    plt.savefig(os.path.join(save_dir, 'convergence_all_runs.png'), dpi=300, bbox_inches='tight')
    print(f"收敛曲线对比图已保存至: {os.path.join(save_dir, 'convergence_all_runs.png')}")
    plt.show()
...
def generate_summary_report(baseline, pso_stats, pso_results, save_dir):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        mse_improvement = ((baseline['test_metrics']['mse'] - pso_stats['mse']['mean']) / 
                          baseline['test_metrics']['mse']) * 100
        
        if mse_improvement > 5:
            f.write(f"PSO优化显著提升了模型性能，MSE平均降低了 {mse_improvement:.2f}%\n")
        elif mse_improvement > 0:
            f.write(f"PSO优化略微提升了模型性能，MSE平均降低了 {mse_improvement:.2f}%\n")
        else:
            f.write(f"PSO优化未能提升模型性能，MSE平均增加了 {abs(mse_improvement):.2f}%\n")
...
def main():
    ...
    if comparison_df is not None:
        ...
        # 保存表格
        comparison_df.to_csv(os.path.join(save_dir, 'metrics_comparison.csv'), index=False)
